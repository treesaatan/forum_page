...
def get_posts():
    complete_posts = []
    # get posts that arent replies | reverse chronological order
    posts = db(db.post.is_reply == None).select(orderby=~db.post.post_date).as_list()
    # get posts that are replies
    for post in posts:
        ...
        post_array = post_array + replies
        complete_posts = complete_posts + post_array
...
def save_post():
    ...
    new_id = db.post.update_or_insert( # if update, new_id = none
        db.post.id == post_id,
        is_reply=is_reply,
        content=content
    )
