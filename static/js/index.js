...
let init = (app) => {
    ...
    app.data = {
        ...
        post_content: "", 
        show_post: false, // new post's text box
        show_reply: null, // null = not replying to any post
        author_name,
        user_email,
    ...
    app.toggle_new_post = (toggle = !app.vue.show_post) => {
        app.clear_new_post();
    ...
    app.do_edit = (post_idx) => {
        app.vue.posts.forEach((post) => (post.edit = false)); 
        app.vue.show_post = false; // turn off new post
        app.vue.show_reply = null;
        let p = app.vue.posts[post_idx];
        p.edit = true;
        p.is_pending = false; // allows loading aspect
    };

    app.do_cancel = (post_idx) => {
        app.clear_new_post();
        let p = app.vue.posts[post_idx]; // post we are editing
        p.edit = false;
        p.content = p.original_content; // original content NEVEr changes
    ...
    app.add_post = () => {
        if(app.vue.post_content.length == 0) { // no empty posts
            return;
        } 
        let q = {
            id: null,
        ...
        axios.post(posts_url, {
            ...
            q = {
                ...q, // spread operator
                id: result.data.id, // creates new object
                editable: true,
                content: result.data.content, // or app.vue.post_content
                original_content: result.data.content,
                server_content: result.data.content,
                author: author_name,
                email: user_email,
            };
            app.vue.posts = [q, ...app.vue.posts]; // newest post q at the front
            app.reindex();
    ...
    app.do_delete = (post_idx) => {
        let p = app.vue.posts[post_idx];
        // TODO:
        // If the post has never been added to the server,            // simply deletes it from the list of posts.
